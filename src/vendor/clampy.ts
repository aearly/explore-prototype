 ...
 * https://github.com/clampy-js/clampy/blob/master/src/clampy.ts
 */
...
export function clamp(
  ...
  function getLineHeight(elem: HTMLElement): number {
    if (options?.lineHeight) return options.lineHeight;
    let lh: any = computeStyle(elem, 'line-height');
    if (lh === 'normal') {
      // Normal line heights vary from browser to browser. The spec recommends
      // a value between 1.0 and 1.2 of the font size. Using 1.1 to split the diff.
      lh =
        parseFloat(parseFloat(computeStyle(elem, 'font-size')).toFixed(0)) *
        1.1;
    }
